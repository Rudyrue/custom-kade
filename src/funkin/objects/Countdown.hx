...
class Countdown extends FunkinSprite {
	...
	public dynamic function onTick(tick:Int):Void {
		switch (tick) {
			case 4: 
				if (!silent) FlxG.sound.play(Paths.sfx('intro3'));
				animation.frameIndex = 0;
			case 3: 
				if (!silent) FlxG.sound.play(Paths.sfx('intro2'));
				animation.frameIndex = 1;
			case 2: 
				if (!silent) FlxG.sound.play(Paths.sfx('intro1'));
				animation.frameIndex = 2;
			case 1: 
				if (!silent) FlxG.sound.play(Paths.sfx('introGo'));
